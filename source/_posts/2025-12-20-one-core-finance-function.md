...
In [the next article](/one-core-defi-function), we build up the minimum implementation of finance using DeFi tooling and study its utility along with other blockchain token application patterns to see if they meet our core definition of finance. We then examine [DeFi primitives for real world assets](/defi-primatives-for-rwa) (RWA) onchain.
